...
from transformers.models.bert.modeling_bert import BertPreTrainedModel # 放弃预训练的网络
...
class ModalEmbeddings(nn.Module):
    ...
    def __init__(self, config, modal_hidden_size, encoder, embeddings):
        ...
        self.proj_embeddings = nn.Linear(modal_hidden_size, config.hidden_size) # 将数据从3072的长度转为 768
    ...
    def forward(
      ...
      # print ("ModalEmbeddings_input_modal_SIZE",input_modal.size()) #ModalEmbeddings_input_modal_SIZE ([15, 9, 224, 224])
        token_embeddings = self.proj_embeddings(self.encoder(input_modal)) 
      #  print ("token_embeddings_size",token_embeddings.shape) #token_embeddings_size ([15, 3, 768])
        seq_length = token_embeddings.size(1)
      #  print ("seq_length",seq_length) #seq_length 3
      ...
      #  print ("token_type_embeddings_shape",token_type_embeddings.shape) #token_type_embeddings_shape torch.Size([15, 5, 768])
        embeddings = token_embeddings + position_embeddings + token_type_embeddings
      #  print ("embeddings_1",embeddings.shape) #embeddings_1 torch.Size([15, 5, 768])
        embeddings = self.LayerNorm(embeddings) 
      #  print ("embeddings_2",embeddings.shape) #embeddings_2 torch.Size([15, 5, 768])
        embeddings = self.dropout(embeddings)
       # print ("embeddings_3",embeddings.shape) #embeddings_3 torch.Size([15, 5, 768])
        return embeddings
...
class MMBTModel(nn.Module, ModuleUtilsMixin):
    ...
    def forward(
        ...
        sequence_output = encoder_outputs[0] #sequence_output 的维度也将是 [batch_size, sequence_length, hidden_size]。这意味着它包含了编码器的最终隐藏状态信息，可以用于后续的任务，例如分类或生成。
        #numpy_sequence_output = [tensor.cpu().numpy() for tensor in sequence_output]
        #print ("numpy_sequence_output",numpy_sequence_output.shape)
        for sublist in sequence_output:
            length = len(sublist)
            #print("sublist_length",length) #sublist_length 9
    
    
        pooled_output = self.transformer.pooler(sequence_output) #pooled_output 可以用于下游任务，例如文本分类，可以将其输入到一个全连接层（或其他适当的层）以生成分类结果。
        for sublist in pooled_output:
            length = len(sublist)
            #print("pooled_output_length",length)     #pooled_output_length 768    
        #numpy_pooled_output = [tensor.cpu().numpy() for tensor in pooled_output]      
        #print ("pooled_output_shape",numpy_pooled_output.shape)
...
#class MMBTForClassification(nn.Module):
    ...
    **hidden_states**: (`optional`, returned when ``output_hidden_states=True``) list of ``torch.FloatTensor`` (one for
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        #print ("input_modal_size",input_modal.shape) #input_modal_size ([15, 9, 224, 224])
        ...
        #print ("logist.shape",logits.shape) #logist.shape torch.Size([15, 14])
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1)) #logits.view(-1, self.num_labels)--loss的维度：logists的m*n/9;维度转换为一维：view(-1)

        if not return_dict:
            output = (logits,) + outputs[2:]
            return ((loss,) + output) if loss is not None else output

        # Optional attention tensor conversion - only if needed for visualization
        # Commented out to prevent memory issues during training/evaluation
        # np.set_printoptions(threshold=np.inf) # 解决了截断问题
        # Attention_12heads=outputs.attentions
        # numpy_Attention_12heads = [tensor.detach().cpu().numpy() for tensor in Attention_12heads]
        
        # print ("numpy_Attention_12heads",numpy_Attention_12heads) #当可视化attention的时候
       
        # with open('Attention_12heads_numpy.txt', 'w', newline='') as file:
        #     for arr in numpy_Attention_12heads:
        #         file.write(str(arr) + '\n')
