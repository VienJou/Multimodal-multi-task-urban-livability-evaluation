...
from MMBT_liva.mmbt_utils_liva_0318 import get_multiclass_labels, get_labels
import pandas as pd
import numpy as np
import json
import os
...
def get_multiclass_criterion(labels_iterable):
    ...
    #return nn.CrossEntropyLoss(pos_weight=label_weights.cuda())
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if wandb_config.multiclass:
                ...
                result = model(b_input_ids,
                               ...
                               attention_mask=b_input_mask,
                               labels=b_labels,
                               return_dict=True) # labels=b_labels, 更改为 None
...
def evaluate(data_loaders_dict, wandb_config, model, prefix="", test=False, criterion=None):
    ...
    for batch in tqdm(eval_dataloader, desc="Batch Evaluating"):
        ...
        out_label_id = b_labels.argmax(dim=1).detach().cpu().numpy()
        preds.append(pred)
        out_label_ids.append(out_label_id)
    print ("preds1",preds)
    print("out_label_ids1",out_label_ids)
    ...
    if wandb_config.multiclass:
        ...
        print ("preds_two",preds)
        print ("out_label_ids_two",out_label_ids)
        ...
        C_matrix = confusion_matrix(out_label_ids, preds)
